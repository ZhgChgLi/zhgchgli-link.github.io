...
class MediumFollowersCountPlugin < Plugin
    ...
    def execute
        medium_host = ENV['MEDIUM_HOST']
        if !medium_host.nil?
            return load_medium_followers(medium_host) || "1K+"
        else
            return "1K+"
        end
    end
    ...
    def load_medium_followers(url)
        ...
        json&.dig(0, "data", "userResult", "socialStats","followerCount")
    end
